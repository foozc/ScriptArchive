...
public class Customize : MonoBehaviour {
	...
	public void Awake()
	{ 
		Body = Mesh.GetComponent<Renderer>().sharedMaterials[0] as ProceduralMaterial;
		Head = Mesh.GetComponent<Renderer>().sharedMaterials[1] as ProceduralMaterial;
		UpdateSubstances();
	}
	...
	public void OnGUI()
	{
		...
		{
			if (!Zoom && !cam.GetComponent<Animation>().isPlaying)
			{
				cam.GetComponent<Animation>()["Zoom"].speed = 1;
				cam.GetComponent<Animation>().Play();
				Zoom = true;
			}
			else if (Zoom && !cam.GetComponent<Animation>().isPlaying)
			{
				cam.GetComponent<Animation>()["Zoom"].speed = -1;
				cam.GetComponent<Animation>()["Zoom"].time = cam.GetComponent<Animation>()["Zoom"].length;
				cam.GetComponent<Animation>().Play();
				Zoom = false;
			}
		}
	...
	private void UpdateSubstances()
	{
		...
		foreach (ProceduralMaterial sub in Mesh.GetComponent<Renderer>().sharedMaterials)
		{

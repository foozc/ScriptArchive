...
public class BuildHelper 
{
	...
	{	
		string outputPath = GenerateOutputPathForBundle(bundle.name, bundle.bundleRelativePath);
...
//#endif
...
#if UNITY_4_2 || UNITY_4_1 || UNITY_4_0
        string error = BuildPipeline.BuildPlayer (  sceneList, 
                                                    outputPath, 
                                                    BuildConfiger.UnityBuildTarget, 
                                                    BuildOptions.BuildAdditionalStreamedScenes | CurrentBuildSceneOpts);
#else
		string error = BuildPipeline.BuildStreamedSceneAssetBundle( sceneList, 
                                                                    outputPath, 
                                                                    BuildConfiger.UnityBuildTarget, 
#if UNITY_WEBPLAYER
                                                                    out crc, 
#endif
                                                                    CurrentBuildSceneOpts);
#endif
#if UNITY_STANDALONE_WIN
        crc = Crc32.GetFileCRC32(outputPath);
#endif
	...
	private static string GenerateOutputPathForBundle(string bundleName, string bundleRelativePath)
	{
        string dicPath = Path.Combine(BuildConfiger.InterpretedOutputPath, bundleRelativePath);
        return Path.Combine(dicPath, bundleName + "." + BuildConfiger.BundleSuffix);
	}
}

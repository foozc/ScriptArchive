...
{ 
    ...
    public sealed class Crc32
    {
        ...
        #region 自定义方法，验证crc
        /// <summary>
        /// 验证下载的文件与配置文件的crc是否一致，一致则返回true
        /// </summary>
        /// <param name="FilePath">文件地址</param>
        /// <param name="crc">配置文件crc码</param>
        /// <returns></returns>
        public bool verifyTwoFile(string filePath, uint crc)
        {
            uint targetFileCRC = GetFileCRC32(filePath);
            if (targetFileCRC == crc)
                return true;
            else return false;
        }
        /// <summary>
        /// 验证下载的文件的Stream流与配置文件的crc是否一致，一致则返回true
        /// </summary>
        /// <param name="stream">文件流</param>
        /// <param name="crc">配置文件crc码</param>
        /// <returns></returns>
        public bool verifyTwoStream(Stream stream, uint crc)
        {
            uint targetStreamCRC = GetStreamCRC32(stream);
            if (targetStreamCRC == crc)
                return true;
            else return false;
        }
        #endregion
